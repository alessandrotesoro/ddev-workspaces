...
mod tests {
    ...
    #[test]
    fn all_four_commands_are_present() {
        let matches = build_cli()
            .try_get_matches_from(["ddev-workspaces", "create", "task-1"])
            .expect("create should parse");

        assert!(matches.subcommand_matches("create").is_some());
        assert!(
            build_cli()
                .get_subcommands()
                .any(|command| command.get_name() == "doctor")
        );
        assert!(
            build_cli()
                .get_subcommands()
                .any(|command| command.get_name() == "list")
        );
        assert!(
            build_cli()
                .get_subcommands()
                .any(|command| command.get_name() == "remove")
        );
    }

    #[test]
    fn create_options_parse_without_derive_macros() {
        let matches = build_cli()
            ...
            .expect("create options should parse");
        let create = matches.subcommand_matches("create").expect("create exists");

        assert_eq!(
            create.get_one::<String>("base").map(String::as_str),
            Some("origin/main")
        );
        assert!(create.get_flag("source-only"));
        assert!(create.get_flag("dry-run"));
    }
}
